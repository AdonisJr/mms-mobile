...
import { fetchUserByType, schedulePreventiveMaintenance } from '../../services/apiServices';
...
export default function SchedulePreventiveMaintenance({ navigation }) {
    ...
    const onSubmit = async () => {
        ...
        try {
            setLoading(true);
            const response = await schedulePreventiveMaintenance(payload);
            Toast.show({ type: 'success', text1: 'Success', text2: 'Preventive maintenance successfully scheduled.' });
            setTimeout(() => {
                navigation.goBack();
            }, 2000)
        } catch (error) {
        ...
        <View className="flex-1">
            <ScrollView className="mt-4 p-4">
                ...
                <TouchableOpacity
                    ...
                    className="mb-2 p-3"
                >
                    <Text>{formData.scheduled_date_from.toLocaleDateString()}</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                    ...
                    className="mb-2 p-3"
                >
                    <Text>{formData.scheduled_date_to.toLocaleDateString()}</Text>
                </TouchableOpacity>
